...
### 📖 Не исправлять код ради стиля, если он до этого проходил все проверки линтерами

В коммитах не должно быть строк, которые были изменены только ради стиля, при условии, если до правок линтеры не ругались на эти строки.

Мотивация: легче читать Пул Реквесты (изменений меньше, диффы чище); снимаются споры, основанные на вкусовых предпочтениях разных разработчиков; избегается бесконечные стилевые правки одного и того же участка разными разработчиками в ходе рефакторингов.


### 📖 Использовать строковую конкатенацию только в простых случаях двух строк

При необходимости составлять сложную строку необходимо использовать строковую интерполяцию `str.format`. Результат будет читаться лучше. В строковом шаблоне сразу видна структура результирующей строки, ее не нужно угадывать по операндам плюсов.
